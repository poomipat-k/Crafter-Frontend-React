...
const Shop = props => {
  const category = props.location.pathname.split("/").slice(-1)[0];

  return (
    <div>
      <ul className={classes.Subnav}>
        <SubNav link="/shop/polo" {...props}>
          Polo
        </SubNav>
        <SubNav link="/shop/tshirt" {...props}>
          T-shirt
        </SubNav>
        <SubNav link="/shop/bag" {...props}>
          Bag
        </SubNav>
      </ul>

      <Switch>
        <Route path="/shop/polo" exact>
          <ShopPage shopItems={DUMMY_SHOP_DATA[category]} />
        </Route>

        <Route path="/shop/tshirt" exact>
          <ShopPage shopItems={DUMMY_SHOP_DATA[category]} />
        </Route>

        <Route path="/shop/bag" exact>
          <ShopPage shopItems={DUMMY_SHOP_DATA[category]} />
        </Route>
